...
import { grayscaleSprite } from '../colorize'
...
export function createCharacter(
  ...
  return {
    ...
    matrixEffectSeeds: [],
    meetingGroupId: null,
    meetingTargetCol: null,
    meetingTargetRow: null,
    drinkTimer: 0,
    sleepMode: false,
    errorTimer: 0,
    opacity: 1,
  ...
  switch (ch.state) {
    ...
    case CharacterState.WALK: {
      ...
      if (ch.path.length === 0) {
        ...
        // Check if arrived at meeting target
        if (ch.meetingGroupId != null && ch.meetingTargetCol != null && ch.meetingTargetRow != null) {
          if (ch.tileCol === ch.meetingTargetCol && ch.tileRow === ch.meetingTargetRow) {
        ...
        // Check if arrived at cooler for drinking
        if (ch.drinkTimer > 0) {
    ...
    case CharacterState.MEET: {
      // Standing at meeting — alternate frames slowly
      if (ch.frameTimer >= TYPE_FRAME_DURATION_SEC) {
        ch.frameTimer -= TYPE_FRAME_DURATION_SEC
        ch.frame = (ch.frame + 1) % 2
      }
      // Exit when meetingGroupId is cleared externally
      if (!ch.meetingGroupId) {
        ch.bubbleType = null
        ch.bubbleTimer = 0
        // Walk back to seat
        if (ch.seatId) {
    ...
    case CharacterState.SLEEP: {
      // Slow frame alternation
      if (ch.frameTimer >= 1.0) {
        ch.frameTimer -= 1.0
        ch.frame = (ch.frame + 1) % 2
      }
      ch.opacity = 0.45
      ch.bubbleType = 'sleep'
      // Exit when isActive becomes true
      if (ch.isActive) {
        ch.sleepMode = false
        ch.opacity = 1
        ch.bubbleType = null
...
export function getCharacterSprite(ch: Character, sprites: CharacterSprites): SpriteData {
  switch (ch.state) {
    ...
    case CharacterState.DRINK:
      return sprites.walk[ch.dir][1]
    case CharacterState.SLEEP:
      return grayscaleSprite(sprites.typing[ch.dir][ch.frame % 2])
    case CharacterState.ERROR:
      return sprites.typing[ch.dir][ch.frame % 2]
    default:
      return sprites.walk[ch.dir][1]
