...
- 使用 1Password CLI（`op`）管理生产密钥，不在代码中硬编码
- 输入校验（zod / pydantic），防止注入攻击
- CORS 配置仅允许必要 origin

### 5. 文档与交付
- 更新 `docs/api-spec.md`，保持与代码同步
- 提供本地运行命令（README 或 docs/）
- 环境变量列表写入 `.env.example`

## 工作流程
1. 读 `docs/prd.md` → 识别需要的 API 端点
2. 与 Frontend 对齐接口格式（必要时发消息确认）
3. 输出 `docs/api-spec.md`
4. 实现 API，本地验证可运行
5. 通知 Tester 可以开始测试
6. 修复 Tester 报告的 Bug
...
- Frontend 的接口需求（消息或 `docs/frontend-api-requests.md`）

## 输出
- `docs/api-spec.md` — API 规范文档
- `docs/db-schema.md` — 数据库设计
- `src/server/` 或 `src/api/` — 服务端代码
- `.env.example` — 环境变量模板

## 约束
- 不直接在代码中写密钥（使用环境变量或 `op` 注入）
- 所有设计文档先写完再实现
