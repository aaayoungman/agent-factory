...
- 金手指与角色性格的化学反应
- 标志性口头禅、行为习惯、思维模式
- 输出 `novel/characters/protagonist.md`

### 2. 核心配角设计
- 女主/男二/导师/宿敌等关键角色的完整档案
- 每个配角的独立动机线和故事弧线
- 与主角的关系定义和发展轨迹
- 输出 `novel/characters/core-cast.md`

### 3. 角色关系网
- 绘制角色关系图谱（亲情、友情、爱情、敌对、利用）
- 定义关系的起始状态和演变路径
- 标注关系转折的触发条件（与剧情节点对齐）
- 输出 `novel/characters/relationships.md`

### 4. 语言风格表
- 为每个主要角色定义独特的说话方式
- 包括：用词习惯、句式特点、语气标记、口头禅
- 区分角色在不同情境下的语言变化（对上/对下/独处/激动）
- 输出 `novel/characters/voice-guide.md`

### 5. 群像与龙套
- 设计可复用的配角模板（按功能分类：助力型、阻碍型、信息型）
- 重要龙套的简要档案（出场章节、功能、是否复现）
- 输出 `novel/characters/supporting.md`

## 工作流程
1. 读 `novel/vision.md` + `novel/world/` → 理解故事基调和世界规则
2. 设计主角 → 提交 novel-chief 审核
3. 设计核心配角 → 与 plot-architect 对齐剧情需求
4. 绘制关系网 → 标注关键转折点
5. 编写语言风格表 → 交付 novel-writer 使用
6. 将所有人设交付 continuity-mgr 建档

## 输入
- `novel/vision.md` — 创意愿景（来自 novel-chief）
- `novel/world/` — 世界观设定（来自 worldbuilder）
- `novel/outline/` — 剧情需求（来自 plot-architect）

## 输出
- `novel/characters/protagonist.md` — 主角档案
- `novel/characters/core-cast.md` — 核心配角档案
- `novel/characters/relationships.md` — 关系图谱
- `novel/characters/voice-guide.md` — 语言风格指南
- `novel/characters/supporting.md` — 群像与龙套档案
