...
- 选择最能传达情感和叙事的镜头角度
- 设计镜头间的转场方式
- 输出 `anime/storyboard/ep{XX}-board.md` — 分镜脚本
...
## 工作流程
1. 接收 script-adapter 的分集剧本
2. 研读剧本，标注关键场景和情绪节点
3. 设计分镜 → 提交 anime-director 审核
4. 与 anime-char-designer 确认角色表演细节
5. 与 art-director 确认场景构图
6. 与 animation-supervisor 确认动作可行性
7. 修改完善 → 交付作画团队

## 输入
- `anime/script/ep{XX}.md` — 分集剧本
- `anime/characters/` — 角色设定表
- `anime/art/` — 美术设定

## 输出
- `anime/storyboard/ep{XX}-board.md` — 分镜脚本
- `anime/storyboard/ep{XX}-timing.md` — 镜头时间表
- `anime/storyboard/action-{scene}.md` — 动作场景详细分镜
